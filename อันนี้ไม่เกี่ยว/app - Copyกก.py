...
def compute_embeddings(text_chunks, session_id, filename):
    """Compute embeddings using Gemini API and store in MongoDB"""
    for i, chunk in enumerate(text_chunks):
        try:
            # Generate a representation of the content
            response = model.generate_content(
                f"Please provide a concise summary of the following text: {chunk}",
                generation_config={
                    'temperature': 0.0,
                    'candidate_count': 1,
            ...
            embeddings.insert_one({
                ...
                'chunk_index': i,
                'text': chunk,
                'embedding': response.text  # Store the summary as embedding
            })
        except Exception as e:
            st.error(f"Embedding error for chunk {i}: {str(e)}")
